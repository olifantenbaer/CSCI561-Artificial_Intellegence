...
def UCS(width, height, start, target, config):
    ...
    while len(frontier)>0:
        ...
        #find min in pathcost
        i=0
        index=0
        temp=frontierCost[0];
        for j in range(len(frontierCost)):
            if(frontierCost[i]<temp):
                temp = frontierCost[i]
                index = i
            i = i+1
        
        node = frontier.pop(index)
        nodeCost = frontierCost.pop(index)
        explored.append(node)
        if node[0]>0:
            # Go UP
            child = [node[0]-1,node[1]]
            childHash = float(str(child[0])+'.'+str(child[1]))
            #child not in frontier and
            if  child not in explored and config[child[0]][child[1]]!=-1:
                if child==target:
                    # Target found
                    found = 1
                    parent[childHash] = node
                ...
                if child in frontier:
                    ...
                    for j in range(len(frontier)):
                        ...
                        i=i+1
                    if(i<len(frontier)):
                        cost = config[child[0]][child[1]] + nodeCost
                        if(cost<frontierCost[i]):
                            fronterCost[i] = cost
                            parent[childHash] = node
                else:
                    frontier.append(child)
                    frontierCost.append(nodeCost+config[child[0]][child[1]])
                    parent[childHash] = node
        if node[0]<height-1:
            # Go DOWN
            child = [node[0]+1,node[1]]
